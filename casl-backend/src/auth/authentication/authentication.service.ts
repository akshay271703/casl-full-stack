...
} from 'src/database/repositories/Permission.repository';
...
export class AuthService {
  ...
  async login(request: LoginRequestDto): Promise<any> {
    ...
    const results: UserPermissionsResponse[] =
      await this.permissionRepo.getUserPermission(user.id);
      console.log(results);
    ...
    return {
      ...plainToInstance(GetUserResponseDto, instanceToPlain(user), {
